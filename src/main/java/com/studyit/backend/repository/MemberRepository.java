...
public interface MemberRepository extends CrudRepository<Member, Integer>{
	@Query("select m from Member m where m.email = :email and m.password = :password and m.delYn = FALSE")
	public List<Member> findByEmailAndPassword(@Param("email") String email, @Param("password") String password);
	
	@Query("select count(m.seq) from Member m where m.email = :email and m.delYn = FALSE")
	public int countByEmail(@Param("email") String email);
	
	@Query("select count(m.seq) from Member m where m.nickname = :nickname and m.delYn = FALSE")
